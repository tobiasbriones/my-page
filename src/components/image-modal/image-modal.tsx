...
export class ImageModal {
  @Prop() modalImage: ModalImage | undefined = undefined;
  @Prop() size: number | undefined = undefined;
  @Event() close!: EventEmitter<void>;
  @Event() previous!: EventEmitter<ModalImage>;
  ...
  render() {
    ...
    const navButtons = (image: ModalImage) => image.index !== undefined && this.size !== undefined && <slot>
      {image.index > 0 &&
          <button class="prev" onClick={() => this.previous.emit(image)}>&#10094;</button>
      }
      {image.index >= 0 && image.index < this.size - 1 &&
          <button class="next" onClick={() => this.next.emit(image)}>&#10095;</button>
      }
    </slot>;
